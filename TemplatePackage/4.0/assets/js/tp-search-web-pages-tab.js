...
( function( $ ) {
	...
	var apiBase = 'browse2-nodoc';
	var apiUrlBase = solrRoot + '/intranet/' + apiBase;
	var newResultsBlock = $( '.webpagesResultsData' );
	...
	var setupPagingListeners = function setupPagingListeners() {
		var $btnPagination = $( '.pagination.webpagesresults a' );
	...
	var buildResults = function buildResults() {
		if ( 0 === documentsResults.response.docs.length ) {
			newResultsBlock.text( 'no web pages results' );
		} else {
			newResultsBlock = $( '.webpagesResultsData' );
			newResultsBlock.empty();
			newResultsBlock.append( '<div class="searchResultsSummary"><strong>' + documentsResults.response.numFound.toLocaleString() + '</strong> web pages returned for <em><b>' + query + '</b></em> </div>' );
			...
			for ( var i = 0; i < documentsResults.response.docs.length; i++ ) {
				...
				moduleTitle = $( '<div />' ).addClass( 'searchResultsTitle lead' ).html( $( '<a />' ).attr( 'href', current.url ).html( boldTerm( current.title, query ) ) );
	...
	var buildPaging = function buildPaging() {
		if ( 20 < documentsResults.response.numFound ) {
			...
			html.push( '<ul class="pagination webpagesresults">' );
	...
	var getResults = function getResults( callback ) {
		...
		$.when( $.ajax( {
			...
			console.log( 'Web pages Request failed: ' + textStatus );
