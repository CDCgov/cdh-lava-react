...
const CDCMarkdown = ({ configUrl, config: configObj, isDashboard = false, isEditor = false, setConfig: setParentConfig }) => {
  // Default States
  ...
  if (loading === false) {
    ...
    content = (
      /* removed from inside div TODO Fix this
        {isEditor && <EditorPanel>{body}</EditorPanel>}
        {!isEditor && body}
*/

      <div className={`cove markup-include ${config.theme}`}>This is a sample content.</div>
    )
  }

  return (
    <ErrorBoundary component='CDCMarkdown'>
