...
### GIFT-EHR Modern Clinical Health Data Lake
...
Purpose:  "To demonstrate how Delta Lake makes it easier to work with large clinical datasets, we will start off with a simple but powerful use case. We will build a dashboard that allows us to identify comorbid conditions (one or more diseases or conditions that occur along with another condition in the same person at the same time) across a population of patients. To do this, we will use a simulated EHR dataset, generated by the Synthea simulator, made available through Databricks Datasets (AWS | Azure). This dataset represents a cohort of approximately 11,000 patients from Massachusetts, and is stored in 12 CSV files. We will load the CSV files in, before masking protected health information (PHI) and joining the tables together to get the data representation we need for our downstream query. Once the data has been refined, we will use SparkR to build a dashboard that allows us to interactively explore and compute common health statistics on our dataset." - per [DataBricks](https://databricks.com/blog/2020/04/21/building-a-modern-clinical-health-data-lake-with-delta-lake.html)

### GIFT-CME Current Market Evaluation Data Lake

The GIFT CME accelerator (Current Market Evaluation) accelerator is based on the data provided by the web site [Current Market Valuation](https://currentmarketvaluation.com/)

Purpose:  This sample demonstrates that GIFT templates can be applied beyond health care to other fields such as finance.  This template will start with another simple but powerful use case.  We will attempt to show that most investors would have higher returns if the followed simple automated methodology to investing rather than a financial advisor.  To do this, we will show the returns and volatility of two sample portfolios 1) A buffet style predined mix of S&P 500 and bond investments and 2) An altered portfolio that adjust investments for extreme scenarios such as 2022 when indicators such as the Buffet Indicator and PE ratios are at high never seen before and 90%-100% or more above their long term avaerages.  If individuals wish, they can compare their own individual returns, particularly if using an individual advisor to see the difference in their gains if they had alternatively used the GIFT approach.

This accelerator is intentionally non public sector and non health care related to show the flexibility of the templates.
