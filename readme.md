...
Purpose:  This accelerator demonstrates how GIFT templates apply beyond health care to other fields such as finance.  This template will start with another simple but powerful use case.  We will attempt to show that most investors would have higher returns if they followed a simple automated methodology to investing rather than using a financial advisor with high fees.  To do this, we will show the returns and volatility of two sample portfolios 1) A buffet style predefined mix of the S&P 500 and bond investments and 2) An altered portfolio that adjusts investments for extreme scenarios such as 2022 when indicators such as the Buffet Indicator and PE ratios are at highs never seen before and 90%-100% or more above their long term averages.  If individuals wish, they can compare their own individual returns, particularly if using an individual advisor to see the difference in their gains if they had alternatively used the GIFT approach.
