...
### Question 10:  Can we use the template if we don't want to change our technoglogy stack?
- Answer:  Yes, the primary value of the GIFT templates are in business process improvements.  Regardless of your technology stack, there is a high probability that 1) There are probably blackholes in your bronze data store where data is input via feeds or data entry and it is never output in a gold data product.  2) There are probably miracles on your executive reports where data labels, captions, aggregates and other elements emerge as miracles in your gold output but are never catlaoged in your bronzed data store 3)  Similarly, there are probably benefits to the business practices on metadata management for discoverability and security that could benefit your organization.
